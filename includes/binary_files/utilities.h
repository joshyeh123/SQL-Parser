...
bool file_exists(const char filename[]) {
  ...
  if (ff.fail()) {
    if (debug)
      cout << "FAILED: file_exists(): File does NOT exist: " << filename
           << endl;
    return false;
  }
  if (debug)
...
void open_fileRW(fstream &f, const char filename[]) throw(char *) {
  ...
  if (!file_exists(filename)) {
    ...
    if (f.fail()) {
      cout << "FAILED: open_fileRW(): File does NOT exist: " << filename
           << endl;
      throw("open_fileRW() failed to open");
    } else {
      if (debug)
        cout << "open_fileRW(): File created successfully: " << filename
             << endl;
    }
  } else {
    // Open the file
